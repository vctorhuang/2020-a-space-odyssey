...
vector_t sdl_get_mouse_position() {
    int x;
    int y;
    SDL_PumpEvents();
    SDL_GetMouseState(&x, &y);
    vector_t xy = {x, WINDOW_HEIGHT-y};
    return xy;
}
...
void render_text(vector_t top_left, const char *text, int font_size) {
    ...
    SDL_Rect dstrect = {top_left.x, top_left.y, texW, texH};
...
void play_sound(char *sound_path) {
    SDL_LoadWAV(sound_path, &wavSpec, &wavBuffer, &wavLength);
...
// Pass the scene in so we can modify it based on keypresses
...
void sdl_draw_circle(double radius, vector_t center, rgb_color_t color){
    //check parameters
    assert(0 <= color.r && color.r <= 1);
    assert(0 <= color.g && color.g <= 1);
    assert(0 <= color.b && color.b <= 1);

    // Since SDL draws y in reverse direction to our coordinate system
    vector_t window_center = get_window_center();
    filledCircleRGBA(renderer, center.x, (2 * window_center.y) - center.y, radius,
                     color.r * 255, color.g * 255, color.b * 255, 255);
}
...
void sdl_render_scene(scene_t *scene) {
    ...
    for (size_t i = 0; i < body_count; i++) {
        ...
        if (radius != 0){
            sdl_draw_circle(radius, center, body_get_color(body));
        }
        if (shape != NULL) {
